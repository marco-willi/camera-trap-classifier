...
def get_synth_input_fn(height, width, num_channels, num_classes):
    ...
    def input_fn(is_training, data_dir, batch_size, *args):
        images = tf.zeros((batch_size, height, width, num_channels),
                          tf.float32)
...
class Model(object):
    ...
    def __call__(self, inputs, training):
        ...
        if self.data_format == 'channels_first':
            # Convert from channels_last (NHWC) to channels_first (NCHW).
...
def resnet_model_fn(features, labels, mode, model_class, num_classes,
    ...
    model = model_class(resnet_size, num_classes, data_format)
    logits = model(features, mode == tf.estimator.ModeKeys.TRAIN)


     # Find class predictions and map to class labels
    class_mapping = tf.constant(class_mappings)
    class_id_predictions = tf.argmax(logits, axis=1)
    class_table = tf.contrib.lookup.index_to_string_table_from_tensor(
            class_mapping, default_value="UNKNOWN")
    class_label_predictions = class_table.lookup(class_id_predictions)


    predictions = {
      'classes': class_id_predictions,
      'class_labels': class_label_predictions,
